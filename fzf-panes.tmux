...
_print_src_line() {
    local fmt='%-6s  %-40s  %b%5s%b%s  %8s  %4s  %4s  %4s  %-8s  %-7s  %s\n'
    local ps_info="$2"
    local pane_info=($1)
...
panes_src() {
    ...
    local cur_id="$1"
    printf "%-6s  %-40s  %6s  %8s  %4s  %4s  %4s  %-8s  %-7s  %s\n" \
        'PANEID' 'WINDOW' 'PANE' 'PID' '%CPU' '%MEM' 'NLWP' 'TIME' 'TTY' 'CMD'
    panes_info=$(tmux list-windows -af '#{!=:#{pane_dead},1}' -F \
        '#D #{s| |_|:session_name}.#{s| |_|:window_name} #I.#P #{?window_zoomed_flag,⬢,❄} #{pane_tty} #{pane_current_path} #T' |
        sed -E "/^$cur_id /d")
    # echo "$panes_info"
